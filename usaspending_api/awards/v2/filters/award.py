...
def award_filter(filters):
    ...
    for key, value in filters.items():
        ...
        if key == "keyword":
            ...
            recipient_qs = Q()
            recipient_list = LegalEntity.objects.values('legal_entity_id').filter(
            ...
            if keyword.isnumeric():
                naics_list = NAICS.objects.filter(code__icontains=keyword).values('code')
            else:
                naics_list = NAICS.objects.filter(
                    description__icontains=keyword).values('code')
            if naics_list.exists():
                naics_qs = Q(latest_transaction__contract_data__naics__in=naics_list)

            psc_qs = Q()
            if len(keyword) == 4 and PSC.objects.filter(code=keyword).exists():
                psc_list = PSC.objects.filter(code=keyword).values('code')
            else:
                psc_list = PSC.objects.filter(description__icontains=keyword).values('code')
            if psc_list.exists():
                psc_qs = Q(latest_transaction__contract_data__product_or_service_code__in=psc_list)
            ...
            piid_qs = Q(piid=keyword)
            fain_qs = Q(fain=keyword)
